...
// the minimal interface for an object which owns a SearchIndex.  At the moment
// it will always be a RefSelectorElement.
interface Renderer {
  render(): void
}
  ...
  search(query: string) {
    ...
    for (const ref of this.knownItems) {
      ...
      if (pos === 0) {
        ...
        prefixes.push(ref)
        if (query === ref) this.exactMatchFound = true
        continue
      }
      result.push(ref)
    }
